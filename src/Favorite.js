...
function Favorite (){
        ...
        <div>
            {
                favorite.map((fav, index) =>(
                    <div className="">
                        ...
                        <button className={`bg-yellow-600 relative bottom-10 p-2 rounded-xl text-white w-16 flex justify-center items-center ${favorite ? "block" : "hidden" }`}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
    ...
    return (
        <section className="flex flex-row gap-2">
            <div>
            <div className="flex justify-center items-center justify-items-center flex-row sm:gap-8 gap-3 mb-10 ">
                <button className="bg-red-600 p-2 rounded-xl text-white sm:w-20 w-10" onClick={() => setPage((prev) => Math.max(prev - 1, 0))} disabled={page === 0 || isFetching}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
                </svg>
                </button>
                <button className="bg-green-600 p-2 rounded-xl text-white sm:w-20 w-10" onClick={() => setPage((old) => (data.length < 10 ? old : old + 1))}
